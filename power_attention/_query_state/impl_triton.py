...
@triton.autotune(fwd_configs, key=["deg", "d", "e", "fused", "D"])
@triton.jit
...
@triton.autotune(dQ_bwd_configs, key=["deg", "d", "e", "fused", "D"])
@triton.jit
...
@triton.autotune(dS_bwd_configs, key=["deg", "d", "e", "fused", "D"])
@triton.jit
...
for tid in range(0, tl.cdiv(c, BLOCK_T)):
    if fused:
        ...
        m = tl.load(p_m, mask=(range_t + tid * BLOCK_T) < c, other=float('inf'))
